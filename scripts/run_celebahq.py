...
# def evaluation(
#     model_dir='diffusion_celebahq_model', tpu_name = None, bucket_name_prefix=None, once=False, dump_samples_only=False, total_bs=128,
...
def evaluation(
    model_dir = 'diffusion_celebahq_model',
    tpu_name=None,  # unused in local run
    bucket_name_prefix=None,  # unused in local run
    once=True,
    dump_samples_only=True,
    total_bs=1,  # generate 1 image for simplicity
    tfds_data_dir='tensorflow_datasets',
):
    import json
    import os

    print(f"Running evaluation from model_dir: {model_dir}")
    
    # Load training arguments
    with open(os.path.join(model_dir, "kwargs.json"), "r") as f:
        kwargs = json.load(f)
    ...
    # Dummy dataset placeholder (only need num_classes)
    class DummyDataset:
        num_classes = kwargs['num_classes'] if 'num_classes' in kwargs else 1
    ds = DummyDataset()
    ...
    dummy_noise = tf.random_normal([1, 256, 256, 3])
    y = tf.constant([0], dtype=tf.int32)

    samples_tensor = model.samples_fn(dummy_noise, y)['samples']

    saver = tf.train.Saver()

    with tf.Session() as sess:
        ckpt = tf.train.latest_checkpoint(model_dir)
        print("Restoring model from checkpoint:", ckpt)
        saver.restore(sess, ckpt)

        samples = sess.run(samples_tensor)
        print("Sample generated! Shape:", samples.shape)

        # Save sample image to disk
        import PIL.Image
        img = ((samples[0] + 1) * 127.5).astype('uint8')  # [-1,1] -> [0,255]
        PIL.Image.fromarray(img).save("sample_output.png")
        print("Saved generated image as sample_output.png")

def train(
    exp_name='celeba_subset\train', tpu_name = locals, bucket_name_prefix = None, model_name='unet2d16b2c112244', dataset='celebahq256',
